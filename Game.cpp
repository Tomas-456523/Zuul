...
//set up the basic game data MARK: initialize
Game::Game(Save*& _save) : save(_save) {} //track the save by reference so we can edit it

//sets up the entire game world, including rooms, npcs, items, and everything else
void Game::SetupWorld() {
	...
	timemachine->setTimeMachine(OUT);
	...
	TimeMachineDirection.emplace_back(TO_THE_VOID, yard);
	...
	Attack* pacupunctuken = new Attack("ACUPUNCTUKEN", "pinned", false, 22, 2, 15, 4, 4, 1);
	...
	Attack* prespoints = new Attack("PRESSURE POINTS", "fired precise needles at", false, 15, 1, 15, 3, 3, 1);
	...
	carlos->addRecruitmentDialogue({{self, "Hey wanna join my team?"}, {carlos, "no"}, {self, "Come on you gotta get some vitamin D."}, {carlos, "no i gotta start over stealing microsofts monies >:("}, {self, "If you want monies we get a lot just by fighting random enemies."}, {carlos, "..."}, {carlos, "fine ill join u"}});
	...
	burgerprisoner->setDialogue({{burgerprisoner, "I may not be able to assist physically,"}, {burgerprisoner, "but I will be praying for you from here."}, {burgerprisoner, "Once you get all three components, just bring them to me so I can put the device together!"}}); //I designed this last line mostly so it's more clear for people who drop the components on the floor before asking him (for some reason)
	...
	shared_ptr<Conversation> prisyes = make_shared<Conversation>(Conversation({{self, "..."},
							...
							{self, "I've tried to get in before but I don't think anybody's ever made it inside."},
							{self, "The door is very impressive technology."},
	...
	shared_ptr<Conversation> lobsavedialogue2 = make_shared<Conversation>(Conversation({{burgerprisoner, "I may not be able to assist physically,"}, {burgerprisoner, "but I will be praying for you from here."}, {burgerprisoner, "Once you get all three components, just bring them to me so I can put the device together!"}})); //just the original dialogue again
	...
	lobappearchange.linkedDescriptions.push({burgerprisoner, "A prisoner gray with age, imprisoned for resisting the global domination of BURGER."}); //he's not chained to the wall anymore since the wall got destroyed and stuff
	...
	WorldChange usetheplotdevice;
	usetheplotdevice.defeatRooms.push({self, abyssrestaurant});
	usetheplotdevice.defeatRooms.push({burgerman, limbo});
	usetheplotdevice.redirectRooms.push({elevatorentrance, abyssentrance});
	usetheplotdevice.redirectRooms.push({elevator, abysselevator});
	usetheplotdevice.redirectRooms.push({elevatortop, abysselevatortop});
	usetheplotdevice.redirectRooms.push({elevatorbottom, abysselevatorbottom});
	usetheplotdevice.redirectRooms.push({BURGERRESTAURANT, abyssrestaurant});
	usetheplotdevice.redirectRooms.push({mainstreet5, mainstreethole}); //update the surface world to account for the lack of stratospheric restaurant
	...
	usetheplotdevice.exitPavings.push(tuple<Room*, Room*, const char*, const char*>(burgbasene, abyss, IN_THE_HOLE, NULL));
	usetheplotdevice.exitPavings.push(tuple<Room*, Room*, const char*, const char*>(burgbasene, abyss, IN_THE_HOLE, NULL));
	usetheplotdevice.exitPavings.push(tuple<Room*, Room*, const char*, const char*>(burgbasene, abyss, IN_THE_HOLE, NULL));
	usetheplotdevice.uncon = CANDISMISS; //can't dismiss your teammates because you're in the abyss and the time machine hasn't arrived yet
	
	//THE PLOT DEVIIIIIIIIIIIIIIIIIIIIIICE!!!!!!!!!!!!!!!!!!!!!!!!!! which you use to get to the final boss
	Item* THEPLOTDEVICE = new PLOTDEVICE("THE PLOT DEVICE", "The device masterfully engineered to do whatever the plot requires.", limbo, BURGERRESTAURANT, 
		...
		{{NULL, "You hand over THE PLOT DEVICE to the BURGER MAN."}, //option a, give the device (bad ending)
		...
		{{self, "Nah I'm good."}, //option b, use the device (go to abyss for good ending)
		 ...
		 {NULL, "The BURGER MAN's BURGERy shell burst into ashes!"},
		 ...
		 {NULL, "You have arrived at your destination."}}, usetheplotdevice); //the lot device does these changes when using it and not giving it

	Item* bpblender = new BlenderItem("", "", limbo, 
		{{self, "Alright I have all the components."},
		 {burgerprisoner, "Nice!"},
		 {NULL, "You hand the OUTPUT ANTENNA, the BIG RED BUTTON, and the PLOTOMETER over to ARCHIBALD."},
		 {burgerprisoner, "Let's see if I can figure this out..."},
		 {NULL, "ARCHIBALD carefully puts together THE PLOT DEVICE's components."},
		 ...
		 {NULL, "ARCHIBALD hands you THE PLOT DEVICE."},
		 {self, "Ayyyy nice."},
	...
	plotchanges.linkedDescriptions.push({burgerman, "The manager and mascot of BURGER. He has a BURGER for a head and an uncanny stature.\nHe seems very still."});
	...
	plotchanges.linkedWelcomes.push({BURGERRESTAURANT, {{NULL, "THE PLOT DEVICE's PLOTOMETER locked onto the BURGER MAN!"}}});
	...
	developer->addConversation({{developer, "Have you ever played BURGER QUEST? (the first one)"},
	 {developer, "The sequel is way better but if you're curious you can try it on this link:"},
	 {developer, "https://codehs.com/sandbox/id/zuul-kb8vTu/run (You're probably on a terminal so you should do shift+insert to copy paste.)"}});
	...
	developer->addConversation({{developer, "Hey Bernard guess how I came up with your name."},
	 {self, "idk"},
	 {self, "If you're writing my dialogue how are you supposed to come up with a natural guess?"},
	 {developer, "That's a good question."},
	 {developer, "Let's see what ChatGPT guesses and that'll be your guess."},
	 {developer, "uh what"},
	 {developer, "Bro said Bernard came from burger + nerd?"},
	 {developer, "It says because you're secretly a burger nerd?"},
	 {developer, "Why would you guess that?"},
	 {developer, "Let's try that again."},
	 {self, "Is it because Bernard is a funny \"real name\" for a video game protagonist?"},
	 {self, "It has that plain, slightly old-fashioned, weirdly specific energy where the character could have been named something heroic,"},
	 {self, "but instead the true canon name is just:"},
	 {self, "Bernard"},
	 {self, "Which makes this line land:"},
	 {self, "{developer, \"Don't worry you will always truly be Bernard.\"}"},
	 {self, "It treats \"Bernard\" like a profound identity reveal even though it is just Bernard."},
	 ...
	 {developer, "since it's BURGER QUEST 2,"},
	...
	cheryl->addConversation({{cheryl, "Hi I'm Cheryl, the developer sent me to come pick you up."},
	...
	aboss->setEnterChanges(abyssescape); //you can leave the abyss after enocuntering the BURGER MENACE whether you lose or win, so you can adjust your team in the case of losing and so you can just leave for both cases
	...
	Effect* extrafire = new Effect("EXTRA ON FIRE", 3, 8);
	...
	NPC* lavaguardian = new NPC("", "LAVA GUARDIAN", "Huge guardian with radiant molten armor and weapons.\nHe appears to have wandered onto the bridge while the lava level was high, and now guards the gate to BURGERSBURG.", limbo, 0, Stats(300, 30, 11, 20, 11, 10, 9), Stats(2, 1, 0, 0, 0, 0, 0));
	...
	Attack* uplash = new Attack("UPSLASH", "slashed", true, 6, 6, 10, 1, 1, 1);
	...
	solarflare->hpmin = 0.35;
	lavaguardian->addSpecialAttack(solarflare);
	Attack* heatwave = new Attack("HEAT WAVE", "washed intense heat over the team", false, 18, 0, 0, 1, 1, 999);
	heatwave->focushits = false;
	heatwave->addEffect(extrafire);
	heatwave->hpmax = 0.35; //heat wave replaces solar flare because solar flare would be extra annoying at this point, whereas heat wave pressures you to finish the fight quickly since you're almost at the end (unlikely to be used at this point but like theoretically)
	lavaguardian->addSpecialAttack(heatwave);
	Attack* gammarayburst = new Attack("GAMMA RAY BURST", "unleashed a burst of gamma radiation", false, 0, 6, 100, 1, 1, 999);
	gammarayburst->focushits = false;
	Effect* burstopen = new Effect("BURST OPEN", 2147483647, 0, 0, 1, 0.25);
	Effect* irradiated = new Effect("IRRADIATED", 2147483647);
	irradiated->damagebuff = 2.0;
	gammarayburst->addEffect(irradiated);
	gammarayburst->selfeffect = burstopen;
	lavaguardian->stageAttack(0.35, gammarayburst);
	...
	Attack* askew = new Attack("ASKEW", "hit", true, -5, 4, 0, 1, 1, 1);
	...
	Attack* doabarrelroll = new Attack("DO A BARREL ROLL", "did a barrel roll at", true, 5, 2, 0, 4, 5, 1);
	...
	Attack* medge = new Attack("MICROSOFT EDGE", "sliced", true, -5, 3, 10, 1, 1, 1); //made of fine chromium
	...
	Effect* searched = new Effect("RESEARCHED", 5, 0, 0, 1, 0.5);
	...
	Attack* yahoo = new Attack("YAHOO!", "went crazy", true, 7, 2, 5, 6, 6, 1);
	...
	Effect* buffering = new Effect("BUFFERING", 3);
	buffering->falldamage = 30;
	iexplorer->addEffect(buffering);
	browser->addSpecialAttack(iexplorer);
	Attack* firefox = new Attack("FIREFOX", "breathed fire upon", false, 14, 5, 20, 1, 1, 3);
	Effect* foxfire = new Effect("FOX FIRE", 3, 7);
	...
	lavaguard->setLeader(true, 15, bridge3, false);
	...
	finalboss->addLinkedConvo(burgerprisoner, {{burgerprisoner, "Hey! I'm glad to see you here."},
		{self, "Yo."},
		{self, "It's good to see you not in prison."},
		{burgerprisoner, "Yeah."},
		{burgerprisoner, "While I was still there, I felt everything shaking,"},
		{burgerprisoner, "and upon investigating, the entire restaurant was falling into the earth!"},
		{burgerprisoner, "I wasn't really sure what to think of it."},
		{burgerprisoner, "But it looks like it turned out alright!"},
		{self, "Yeah that was just THE BURGER MENACE dragging the restaurant into the Abyss."},
		{self, "Because he was salty I plot device'd the BURGER MAN."},
		{self, "But then I plot device'd him too so yeah."},
		{burgerprisoner, "I see."},
		{burgerprisoner, "Well, I'm glad that BURGER has finally been dealt with."},
		{self, "So you're the priest here then?"},
		{burgerprisoner, "Yeah."},
		{burgerprisoner, "It's good to be back,"},
		{burgerprisoner, "though it's been far too long."},
		{burgerprisoner, "I have a lot of catching up to do with this village."},
		{burgerprisoner, "Also, who knew we lived in the same village?"},
		{burgerprisoner, "What a coincidence!"},
		{burgerprisoner, "Well, if you need anything, just let me know!"}});
	finalboss->addLinkedDialogue(burgerprisoner, {{burgerprisoner, "Hey! What's up?"},
		{self, "Nothing much."},
		{burgerprisoner, "Well, thanks for stopping by."}});
	finalboss->addLinkedTitle(burgerprisoner, "FATHER"); //he was the missing priest from the village church
	...
	finalboss->addLinkedConvo(archie, {{archie, "Hey!"},
		{archie, "Did you see what happened to the BURGER RESTAURANT?"},
		{archie, "It's kind of just..."},
		{archie, "not there anymore?"},
		{self, "Oh yeah BURGERs are actually evil,"},
...
void Game::PrintRoomData(Room* currentRoom, bool track) { //only track it if the player typed the ROOM command, not if this was called due to traveling
	...
	if (NPC* pursuer = self->getPursuer()) { //print pursuer logic if needed
	...
	//if we're walking down the infinite hallway in Henry Jerry's house
		...
		if (!strcmp(direction, "WEST")) { //we're going east which decrements the universe number
			if (currentUniverse) currentUniverse--; //decrement the universe unless we're at the 0th universe, because there is no west direction there but you think there is
				...
				cout << "\nThe WEST exit doesn't exist. You thought there was a door, but you just ran into a wall.";
				return; //return because we didn't go anywhere
			}
		} else { //if we're going to the same room in this case, it's guaranteed to be that we're going east
			currentUniverse++; //going east increments universe number; universe number is not clamped in the positive direction
		}
	}
...
void Game::fight(Room* currentRoom, const char* name, bool track) {
	...
	if (battlestatus == 0) { //lose
		...
		if (npc->getLobster()) { //if it was the lobster
			...
			cout << "\n" << setw(strlen(self->getName())) << ""; //spacing
			cout <<                                    "                                                 (type your lobster's name here!)";
			cout << "\n" << self->getName() << " - \"Oh nice a pet lobster! I think I'll name you ";
			...
			if (!strcmp(name, "")) { //Bernard really wanted to name the lobster so he expresses his disappointment if the player just ENTERed immediately and didn't type a name
				cout << self->getName() << " - \"Ok nevermind I guess I won't name you.\"";
				CinPause();
			...
			if (!strcmp(name, "FLORIAN")) {
				cout << "\n" << self->getName() << " - \"Yeah that's what I was thinking too!\"";
			...
			if (WorldState[IMPRISONED]) { //if this is the escape sequence
				...
				CinPause();
				Conversation c = {{self, "Nice!"},
					{self, "Hey are you coming?"},
					{NULL, "ARCHIBALD - \"No,\""},
			...
			//we do this until the player uses THE PLOT DEVICE!
			bool promptline = true; //if the prompting > should be in a newline, only false for typing nothing
			for (bool continuing = true; continuing;) { //loops until valid response, after which the loop is returned out of
			...
			npc->printDialogue(true); //the final boss has another defeat convo, which is the final ending cutscene stuff, so print that. After that, we're done! That's the whole game!
...
void Game::takeItem(Room* currentRoom, const char* itemname) {
	...
	if (!strcmp(item->getType(), "escapeorb") && party->size() < 4) { //escape orbs must be taken with 4 people so that the temples can actually use their themes and stuff 
		size_t partysize = party->size();	//here we say the together for emphasis unless it's just you
	...
	if (!strcmp(item->getType(), "manhole")) {
		...
		travel(currentRoom, NULL, true, orb->getDestination()); //go into the temple
...
void Game::dropItem(Room* currentRoom, const char* itemname) {
	...
	if (!strcmp(item->getType(), "PLOTDEVICE")) { //if the player can drop the device, then I have to deal with more states. It's prolly more responsible to not drop the device, anyway!
	...
	if (!strcmp(item->getType(), "weapon")) { //weapon items enable the use of their move
		...
		if (!orb->getInert()) { //make it reset the temple plus a pause, but you can just drop stone orbs normally
			...
			travel(currentRoom, NULL, true, orb->getEntrance()); //go back to the temple entrance
...
void Game::useItem(Room* currentRoom, const char* itemname) {
	...
	if (lobster != NULL) { //checks the lobsteriness of the potential lobster
		...
		travel(currentRoom, NULL, true, lobster->getHome());
		return;
	}
	...
	if (!strcmp(itemname, "")) { //error message if the player didn't clarify what exactly to use
		...
		npc = self; //if we only have the player in the party we just use them because who else would we be targetting
	}
	...
	//teaches the player character new attacks
		...
		for (size_t i = 0; i < attacks.size();) { //move all currently unusable attacks to the future attacks vector
			if (attacks[i]->minLevel > player->getLevel()) {
				futureattacks.push_back(attacks[i]);
				attacks.erase(attacks.begin() + i); //remove it because it was usable
			} else i++; //only advance if we don't delete anything
		}
		...
		for (size_t i = 0; i < amount; i++) { //add and describe all the new attacks we learned
			...
			if (i < amount-1) CinPause(); //do not print the final pause cause then the > looks bad
		}
			...
			if (treasure->getItems().size()) { //if the treasure had an item
				...
				for (size_t i = 0; i < amount; i++) {
					if (!strcmp(items[i]->getType(), "weapon")) {
						...
						applyWeaponAttack(self, items[i]);
					}
				}
			}
		}
	...
	//blender items blend a group of items into a new item
		...
		for (const char* ingredient : blender->getIngredients()) { //check if weave all the ingredients by sorting them into have and not have
			if (Item* item = getItemInVector(inventory, ingredient)) { //try to find the ingredient
				foundingredients.push_back(item); //we found it! so add it to the list of found ingredients
			} else { //we didn't find the ingredient so we add it to the list of missing ingredients
				missingingredients.push_back(ingredient);
			}
		}
	...
	//light and cold orbs get slightly more detailed instructions than material items
		...
		return;
	} else if (!strcmp(item->getType(), "PLOTDEVICE")) {
		PLOTDEVICE* device = (PLOTDEVICE*)item;
		if (!device->getUsable(currentRoom)) { //if you can't currently use the plot device say why
			cout << "\nTHE PLOT DEVICE's PLOTOMETER isn't locked onto anything!";
			return;
		} //print the first part of the conversation which will the branch depending on the player's choice which is hopefully to not give the device
		printConversation(&device->getUseText(), false);
		if (AOrB(NULL, "YES", "NO")) { //yes choice - give the device to the BURGER MAN (bad ending)
			printConversation(&device->getChoiceText(false), true);
			WorldState[GAMEEND] = true;
			return; //return so we don't log it as use because we didn't fully use the item
...
void Game::recruitNPC(Room* currentRoom, const char* npcname) {
	...
	if (newpartysize > 4 && !npc->getFifth()) {
		cout << "\nYour party is full!";
		return;
	}
...
void Game::dismissNPC(Room* currentRoom, const char* npcname) {
...
//prints anything the targeted npc has to say, and other ASK-related functionalities MARK: print dialogue
void Game::printNPCDialogue(Room* currentRoom, const char* npcname) {
	...
	if (npc->getInternalBlender() && npc->blendItems(&inventory)) { //if we successfully craft something, all the dialogue and item processes have been handled already, so we return and don't print the normal dialogue
		return;
...
void Game::printInventory() {
	cout << "\nYou have " << mony << " mon" << (mony == 1 ? "y" : "ies") << ", and you"; //prints mony amount
...
void Game::analyze(Room* currentRoom, const char* name) {
...
bool Game::saveWorld(time_t& savetime, const char* andwhat, bool& laws, bool gameend) { //laws = last action was save
	...
	Save::SaveGame(save, mony, savetime); //save the game!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
bool Game::confirmQuit(time_t savetime, bool laws) { //laws = last action was save, because "ahhhhh you will lose 3s of progress!!!!!!!!!!!!!!!! don't quit without saving nooooooooooo!!!!!!!!!!!!!!"
...
void Game::play() {
	cout << "\nLoading..."; //print loading text in case setting up the game takes a noticeable amount of time

	emptyExterns(); //reset all the stuff in helper so the game session doens't have garbage data

	SetupWorld(); //sets up the game world
	...
	if (WorldState[BURGERMENDEF]) { //if you've beaten the game, some of the help texts are different
		flavorText[0] = "You practice your breakdancing skills now that you're done with your BURGER QUEST.";
		flavorText[5] = "You accidentally find spoilers for the ending of BURGER QUEST 2. Good thing you already beat it.";
		flavorText[7] = "You spot a billboard advertising literally nothing.";
		flavorText[13] = "You realize how you've never seen a normal burger.";
	}
	...
	//we continue until continuing is set to false (when the player quits or gets an ending)
	for (bool continuing = true; continuing;) { //the main loop!
		char command[255] = ""; //the charray that the player inputs into
		...
		if (!strcmp(commandWord, "GO")) { //for going in a direction
			...
			continuing = saveWorld(savetime, &commandExtension[0], savedlast);
		} else if (!strcmp(commandWord, "QUIT")) { //for quitting the game without saving
			continuing = confirmQuit(savetime, savedlast);
