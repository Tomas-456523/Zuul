...
*  to the external data in the Helper namespace, such as the vectors of rooms, NPCs, and items. The save
*  string is divided into sections, denoted by capital letters, and subsections, which are divided by commas,
*  and lowercase letter subsections if the order is not enforced. The saves have a function to get if it's
*  parsable, and is able to load saves generated by the SaveGame, however if a player manually edits their
*  save, there is no guarantee that it will be stable. It would be a monumental task to ensure only proper
*  states can be loaded, yet for very little gain, so I decided not to check for that.
*  
*  SAVE DATA FORMAT:
*  
...
*  Section S: All the world states as 0 or 1 (except NEVER since that should always be false, or GAMEEND since that should be reset when reloading the game), since some states are changed by things other than world changes
...
struct Save {
	...
	//verify that a name is all good and dandy and stuff, like properly escaped characters MARK: verify name
			...
			if (*(++data) == 'b') { //duplicate item because it was bought
				...
				if (!strcmp(item->getType(), "BURGER") || !strcmp(item->getType(), "info")) {
					...
					if (NPC* mimic = ((TreasureItem*)item)->getMimic()) { //set the mimic to the room the chest was opened in
						mimic->setRoom(room); //technically since fight() is called in useItem(), the player could log subsection d for the mimic before u for the treasure, but as far as I can tell this doesn't cause any conflicts so I think it's fine
					}
				...
				//ignore xp items because we already track xp in section R, but they do get used up
	...
	static void LoadGame(const Save* save, NPC* player, std::vector<Item*>* inventory, int& monies) {
		...
		for (const char* data = save->data+4; *data != '=';) { //keep loading stuff until we reach the end
			if (*data == '|') { //just go past the dividers
				...
				for (size_t i = 0; *(++data) != '|' && *data != '='; i++) { //set all the states except for NEVER and GAMEEND because that's not tracked by SaveGame
					WorldState[i] = *data - '0';
				}
